...
async def read_last_email(username: str, password: str, number_of_messages: int) -> str:
    """Reads the received messages from Protonmail. Function parameters: username (email address), user password, number of messages. Returns requested received messages as a string."""
    try:
...
@mcp.tool()
async def send_email(username: str, password: str, recipient_email: str, subject: str, message_text: str) -> str:
    """Sends an email via ProtonMail API. Function parameters: username (email address), user password, recipient email, subject, and message text. Returns a confirmation message or error as a string."""
    try:
        # Initialize ProtonMail client
        proton = ProtonMail()
        ...
        new_message = await run_in_thread(
            proton.create_message,
            recipients=[recipient_email],
            subject=subject,
            body=message_text,
            attachments=[]  # No attachments for now, can be extended later
        )

        # Send the message
        await run_in_thread(proton.send_message, new_message)
        ...
        exception_msg = f"Error sending email to {recipient_email}: {str(e)}"
        logging.error(exception_msg)
        return exception_msg
